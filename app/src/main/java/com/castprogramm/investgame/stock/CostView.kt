...
class CostView(context: Context, atrr: AttributeSet): GraphView(context, atrr), Up {
    ...
    fun addStock(stock: Stock, live:LifecycleOwner){
        val newPoint : LiveData<MutableList<DBDataPoint>> = stock.costsofStock // Копия LiveData из переменной stock
        ...
        newPoint.observe(live,  { t ->
            val function = LineGraphSeries<DataPoint>(Array(t.size, { t[it] })) // Создание серии значений
            this.addSeries(function) // Добавление серии значений в график
